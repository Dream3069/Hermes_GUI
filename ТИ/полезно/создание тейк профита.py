# Как создать ордер на покупку с тейкпрофитом
import logging
...
def main():
    # Как создать ордер на покупку с тейк-профитом
    with Client(TOKEN) as client:
        ...
        figi = "TCS10A101X50"  # BBG004730ZJ9 - VTBR / BBG004730N88 - SBER

        # получение последней цены на инструмент
        last_price = (
        ...
        # установка процента, на который будет выставляться стоп-ордер с тейк-профитом
        # должна быть установлена ниже последней цены
        percent_down = 5

        # расчет цены для стоп-ордера takeprofit
        calculated_price = last_price - last_price * Decimal(percent_down / 100)
        print(f"calculated_price = {calculated_price}")

        # получение минимального приращения цены и количества цифр после запятой
        ...
        # расчет цены на инструмент, который
        # делится на минимальное увеличение цены
        calculated_price = (
        ...
        # создание ордера на покупку с фиксированной прибылью
