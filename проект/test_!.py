...
from список_активов import get_all_trading_symbols
from график_свечи import plot_candlestick
from портфель_ыы import ii
from хелпер import TradingHelper
Builder.load_string("""
...
class Row(BoxLayout):
    ...
    def __init__(self, name,grafic,profit, **kwargs):
        ...
        if profit > 0:
            ...
            self.color = 1, 0, 0, 1
        g = TradingHelper()
        g.analyze_symbol(name)
        if g==1:
            self.buy_sell = 'рекомендуем к продаже'
        elif g==-1:
            self.buy_sell = 'рекомендуем к покупай'
        else:
